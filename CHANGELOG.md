...
- Serialized thread mutations with a cross-process storage lock and atomic file replacement so concurrent `create`, `add`, `resolve`, `reopen`, and `purge` operations no longer lose messages or leave malformed `.threads.jsonl` lines

## [0.4.4] - 2026-03-16

### Changed

- Reissued the inspection-only GUI rename as a clean sequential release so the latest git tag, GitHub release, npm package, and install verification all align

## [0.4.3] - 2026-03-16

### Changed

- Renamed the remaining browser UI command to `inspect-gui` and repositioned it as a developer/admin inspection surface instead of a general-purpose GUI
