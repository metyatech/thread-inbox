 ...
 *
 *   .thread-inbox-manager.json   — session state (idle/busy, session ID, PID)
 *   .thread-inbox-queue.jsonl    — persistent message queue
 *
 * Key design rules:
...
export const MANAGER_SESSION_FILE = '.thread-inbox-manager.json';
export const MANAGER_QUEUE_FILE = '.thread-inbox-queue.jsonl';
...
export function workspaceKey(dir: string): string {
  return createHash('sha256').update(resolvePath(dir)).digest('hex').slice(0, 16);
}
...
export async function writeSession(dir: string, session: ManagerSession): Promise<void> {
  const filePath = sessionFilePath(dir);
  const key = `session:${resolvePath(dir)}`;
  await withWriteLock(key, () => atomicWrite(filePath, JSON.stringify(session, null, 2)));
}
...
export async function writeQueue(dir: string, entries: QueueEntry[]): Promise<void> {
  const filePath = queueFilePath(dir);
  const content = entries.map((e) => JSON.stringify(e)).join('\n');
  const key = `queue:${resolvePath(dir)}`;
  await withWriteLock(key, () => atomicWrite(filePath, content ? content + '\n' : ''));
}
 ...
 */
export function parseClaudeOutput(stdout: string): { text: string; sessionId: string | null } {
 ...
 */
export async function processNextQueued(dir: string, resolvedDir: string): Promise<void> {
  ...
  if (session.status === 'busy' && session.pid !== null && !isPidAlive(session.pid)) {
    await writeSession(dir, { ...session, status: 'idle', pid: null, currentQueueId: null });
  ...
  const prompt = buildClaudePrompt(next.content, next.threadId, resolvedDir, isFirstTurn);
  ...
  const finish = async (code: number | null): Promise<void> => {
    ...
    try {
      ...
      if (code === 0 && stdout.trim()) {
        ...
        if (text) {
          await addMessage(resolvedDir, next.threadId, text, 'ai', MANAGER_REPLY_STATUS);
        }
        await writeSession(dir, {
          ...currentSession,
          status: 'idle',
      ...
      try {
        ...
        await writeSession(dir, { ...s, status: 'idle', pid: null, currentQueueId: null });
      } catch {
        /* ignore */
      }
    }
  ...
  proc.on('error', async (err: NodeJS.ErrnoException) => {
    ...
    try {
      await addMessage(resolvedDir, next.threadId, notFoundMsg, 'ai', 'needs-reply');
    } catch {
      /* ignore */
    }
...
export async function startBuiltinManager(
  ...
  const session = await readSession(dir);
  if (session.status === 'not-started') {
    await writeSession(dir, { ...session, status: 'idle', startedAt: new Date().toISOString() });
  }
