...
const MANAGER_AUTH_STORAGE_KEY =
  window.MANAGER_AUTH_STORAGE_KEY || `thread-inbox.manager-token:${GUI_DIR}`;
...
async function apiFetchWithToken(
  ...
  if (token) {
    headers.set('X-Thread-Inbox-Token', token);
  }
...
function makeBubble(msg: Msg): HTMLDivElement {
  ...
  senderBadge.className = 'bubble-sender ' + (isAi ? 'bubble-sender-ai' : 'bubble-sender-user');
...
class SectionController {
  ...
  toggle(): void {
    ...
    if (!this.#collapsed) {
      ...
      this.update(this.#lastThreads, this.#lastOpenThreadId, this.#lastOnSelect);
    }
  }
  ...
  update(
    ...
    if (this.#countEl) {
      this.#countEl.textContent = threads.length > 0 ? `(${threads.length})` : '';
    }
  ...
  #buildRow(
    ...
    row.className = 'thread-row' + (openThreadId === thread.id ? ' selected' : '');
  ...
  #patchRow(row: HTMLElement, thread: Thread, openThreadId: string | null): void {
...
class TaskSectionController {
  ...
  #buildRow(task: Task): HTMLDivElement {
    ...
    stageBadge.style.cssText = 'background:#0f3460;color:#93c5fd;border:1px solid #1e40af;';
  ...
  #patchRow(row: HTMLElement, task: Task): void {
    ...
    if (stageBadge) {
      ...
      if (stageBadge.textContent !== newStage) stageBadge.textContent = newStage;
    }
...
class ReplyForm {
  ...
  constructor(thread: Thread, app: ManagerApp) {
    ...
    managerBtn.title = 'メッセージをスレッドに追加し、マネージャーAIにも送信します（Ctrl+Enter）';
    ...
    needsReplyBtn.title = 'メッセージを送り、ステータスを「needs-reply」に変更する';
    needsReplyBtn.addEventListener('click', () => void this.#doSendWithStatus('needs-reply'));
    ...
    reviewBtn.addEventListener('click', () => void this.#doSendWithStatus('review'));
    ...
    const updateSenderBtns = () => {
      userBtn.className = 'opt-btn ' + (selectedSender === 'user' ? 'opt-user' : 'opt-inactive');
      aiBtn.className = 'opt-btn ' + (selectedSender === 'ai' ? 'opt-ai' : 'opt-inactive');
    };
    ...
    const statusBtns = statusOptions.map((opt) => {
      ...
      btn.className = 'opt-btn ' + (opt.value === '' ? 'opt-status' : 'opt-inactive');
    ...
    const updateStatusBtns = () => {
      statusBtns.forEach((btn) => {
        btn.className =
          'opt-btn ' + (btn.dataset.value === selectedStatus ? 'opt-status' : 'opt-inactive');
      });
    };
    ...
    this.#doSend = async () => {
      ...
      this.#setBusy(true);
      const payload: Record<string, string> = { dir: GUI_DIR, content, from: selectedSender };
    ...
    this.#doSendWithStatus = async (status: string) => {
      ...
      const res = await app.apiFetch(`/api/threads/${thread.id}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ dir: GUI_DIR, content, from: 'user', status }),
      });
      if (!res) {
        this.#setBusy(false);
        return;
    ...
    this.#doSendToManager = async () => {
      ...
      this.#setBusy(true);
      const addMessageRes = await app.apiFetch(`/api/threads/${thread.id}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ dir: GUI_DIR, content, from: 'user', status: 'waiting' }),
      });
      if (!addMessageRes) {
        this.#setBusy(false);
        return;
      ...
      const sendRes = await app.apiFetch('/api/manager/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ dir: GUI_DIR, threadId: thread.id, content }),
      });
      if (!sendRes) {
        this.#setBusy(false);
        return;
  ...
  static patchPending(formEl: Element | null, thread: Thread, pendingId: string | null): void {
    if (!formEl) return;
    const managerBtn = formEl.querySelector<HTMLButtonElement>('[data-manager-btn]');
    const pendingNote = formEl.querySelector('[data-pending-note]');
    const isPending = pendingId === thread.id;
    if (managerBtn) {
      managerBtn.disabled = isPending;
      managerBtn.textContent = isPending ? '🤖 返信待ち中...' : '🤖 マネージャーに送る';
    }
    if (pendingNote) {
      if (isPending) {
...
class DetailController {
  ...
  #patch(thread: Thread): void {
    ...
    const badge = this.#detailEl.querySelector<HTMLElement>('[data-detail-badge]');
    ...
    const existingActionBtn = this.#detailEl.querySelector<HTMLElement>('[data-action-btn]');
    if (existingActionBtn) {
      const wasResolved = existingActionBtn.dataset.actionBtnResolved === 'true';
      const isResolved = thread.status === 'resolved';
      if (wasResolved !== isResolved) {
        const newActionBtn = this.#buildActionBtn(thread);
        existingActionBtn.parentNode?.replaceChild(newActionBtn, existingActionBtn);
      }
    }
    ...
    if (msgArea && hasNewMessages) {
      ...
      if (shouldScrollToBottom({ isFirstRender: false, hasNewMessages: true, wasNearBottom })) {
        setTimeout(() => {
          msgArea.scrollTop = msgArea.scrollHeight;
        }, 0);
      }
  ...
  #buildActionBtn(thread: Thread): HTMLButtonElement {
    ...
    if (thread.status === 'resolved') {
      ...
      actionBtn.addEventListener('click', () => {
        void (async () => {
          const res = await this.#app.apiFetch(`/api/threads/${thread.id}/reopen`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
      ...
      actionBtn.addEventListener('click', () => {
        void (async () => {
          const res = await this.#app.apiFetch(`/api/threads/${thread.id}/resolve`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ dir: GUI_DIR }),
          });
...
class ManagerApp {
  ...
  constructor() {
    const sectionKeys = ['ai-replied', 'needs-reply', 'review', 'waiting', 'idle'];
  ...
  init(): void {
    ...
    document.addEventListener('click', (e: MouseEvent) => {
      const header = (e.target as Element | null)?.closest('[data-section-key]');
      if (!header) return;
    ...
    const titleInput = document.getElementById('new-thread-title') as HTMLInputElement | null;
    if (titleInput) {
  ...
  async apiFetch(input: string, init: RequestInit = {}): Promise<Response | null> {
    try {
  ...
  async loadAll(): Promise<boolean> {
    try {
      const [threadsRes, tasksRes] = await Promise.all([
        this.apiFetch(`/api/threads?dir=${encodeURIComponent(GUI_DIR)}`),
        this.apiFetch(`/api/tasks?dir=${encodeURIComponent(GUI_DIR)}`),
      ]);
      if (!threadsRes || !tasksRes) {
        return false;
      }
      if (threadsRes.ok) this.allThreads = (await threadsRes.json()) as Thread[];
      ...
      if (this.managerPendingThreadId) {
        const pt = this.allThreads.find((t) => t.id === this.managerPendingThreadId);
        if (
          !pt ||
          (pt.messages &&
  ...
  async loadManagerStatus(): Promise<boolean> {
    try {
      ...
      const dot = document.getElementById('manager-status-dot') as HTMLElement | null;
      const text = document.getElementById('manager-status-text') as HTMLElement | null;
      const startBtn = document.getElementById('manager-start-btn') as HTMLElement | null;
      if (data.running) {
        ...
        if (text) {
          ...
          text.textContent =
            (isBusy ? 'マネージャーが返答を作成中です' : 'マネージャーは待機中です') +
        ...
        if (text) {
          ...
          text.textContent = 'まだ始まっていません。起動するか、そのまま送信してください';
        }
        ...
        if (text) {
          ...
          text.textContent =
            '止まっています。起動すると再開できます' + (data.detail ? ' — ' + data.detail : '');
        }
        startBtn?.classList.remove('hidden');
      }
    } catch {
  ...
  #renderAll(): void {
    ...
    for (const key of ['ai-replied', 'needs-reply', 'review', 'waiting', 'idle']) {
      this.#sectionControllers[key].update(groups[key], this.openThreadId, onSelect);
    }
  ...
  #showNewThreadForm(): void {
    ...
    setTimeout(
      () => (document.getElementById('new-thread-title') as HTMLElement | null)?.focus(),
      0,
    );
  }
  ...
  async #submitNewThread(sendToManager: boolean): Promise<void> {
    ...
    if (res?.ok) {
      ...
      this.#hideNewThreadForm();
      if (sendToManager) {
        const addMessageRes = await this.apiFetch(`/api/threads/${thread.id}/messages`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ dir: GUI_DIR, content: title, from: 'user', status: 'waiting' }),
        });
        if (!addMessageRes) return;
        const sendRes = await this.apiFetch('/api/manager/send', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ dir: GUI_DIR, threadId: thread.id, content: title }),
        });
        if (!sendRes) return;
  ...
  async #doStartManager(): Promise<void> {
    const btn = document.getElementById('manager-start-btn') as HTMLButtonElement | null;
    const text = document.getElementById('manager-status-text') as HTMLElement | null;
  ...
  async #bootAfterAuth(): Promise<void> {
    const [dataOk, statusOk] = await Promise.all([this.loadAll(), this.loadManagerStatus()]);
    if (!dataOk && !statusOk) {
      return;
    }
    this.#startPolling();
  ...
  async #unlockAuth(): Promise<void> {
    const input = document.getElementById('auth-token-input') as HTMLInputElement | null;
    const submitBtn = document.querySelector<HTMLButtonElement>('[data-action="unlock-auth"]');
  ...
  #showAuthPanel(message = ''): void {
    ...
    document.querySelectorAll<HTMLElement>('[data-auth-content]').forEach((el) => {
